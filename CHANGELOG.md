...
---
### 追加
- `Blue-Page.html`:私的ページ「青いページ」を新規作成(市役所への戻りリンクあり、トップ等からの導線なし)
### 変更
- `about/sitemap.html`:「その他」カテゴリを新設し、「青いページ」を追加
- `data/search-index.json`:「青いページ」を「その他」カテゴリで追加
### 修正
- `about/gaiyou.html`:「ピコぬ市検定（中級）」へのリンクのタイプミス(`piconu-quiz-cyukyu.html`→`piconu-quiz-chukyu.html`)を修正
---

## 2026-08-30（4）
---
### 追加
- `about/piconu-quiz-chukyu.html`:「ピコぬ市検定（中級）」を新規作成。全10問、サイト内の実際のページ内容(CHANGELOGではなく公開ページ)を出題根拠として作成
- `images/banners/banner-quiz2.jpg`:バナー画像を追加
### 変更
- `about/piconu-quiz.html`(初級):タイトル画面に中級検定への導線を追加
- `about/piconu-freegame.html`:ゲーム一覧に「ピコぬ市検定（中級）」を追加
- `about/sitemap.html`・`data/search-index.json`・`about/gaiyou.html`:「ピコぬ市検定（中級）」を追加
---

## 2026-08-30（3）
---
### 変更
- `game/piconu-match3-game.html`:ピコぬくんのセリフをひらがな多めの柔らかい口調に修正
---

## 2026-08-30（2）
---
### 修正
- `game/piconu-match3-game.html`:スマホでタイルが盤面(アクア・ティール)からはみ出す不具合を修正。盤面・タイルを画面幅に応じて自動で縮む流動サイズに変更
- `game/piconu-match3-game.html`:パズル画面の背景が暗すぎた点、コンテナ・タイルの背景が白すぎた点を調整(パズル画面をY2Kパープル系`#8888D8`に、コンテナ・タイル背景をうっすらピンク系に変更)
---

## 2026-08-30（1）
---
### 追加
- `CollarGuide.md`:平成レトロ配色のカラーパレット集を新設(その後、拡張子なしから`.md`にファイル名変更)
### 変更
- `game/piconu-match3-game.html`:ピコぬくんのアバターを絵文字+テキスト表示からSVGデザインに変更(タイトル画面・クリア画面とも)。あわせて、不要になった旧デザイン用CSS(`.pikonu-avatar`箱型・`.pikonu-face`・`.pikonu-screen`等)を削除
- `game/piconu-match3-game.html`:配色を「ファンシー＆Y2Kポップ」(`CollarGuide.md`案1)に変更。バブルガム・ピンク/ライムグリーン/ペリウィンクル・ブルー/アクア・ティール/ヴィヴィッド・オレンジを使用
---
...
### 変更
- `about/gendaibyo-soudan-game.html`:症例一覧にNo.013を追加
---

## 2026-08-27
---
### 追加
- `about/piconu-freesozai.html`:「ピコぬくん素材置き場」を新規作成。ピコぬくんのアイコン素材(images/freesozai/、全10種)を平成初期の個人ホームページ風デザインで公開
### 変更
- `about/sitemap.html`:「ピコぬくん素材置き場」を市の概要の階層に追加
- `about/piconu-kun.html`:サイドバー「関連情報」に「ピコぬくん素材置き場」へのリンクを追加
- `data/search-index.json`:「ピコぬくん素材置き場」を追加
---

## 2026-08-26（5）
---
### 追加
- `game/012-nukapiko.html`:現代病相談窓口記録ファイルのシナリオゲーム「ぬかピコの話」を追加(計12本に)
### 変更
- `about/gendaibyo-soudan-game.html`:症例一覧にNo.012を追加
---

## 2026-08-26（4）
---
### 変更
- `facilities/shoten.html`:記録町商店街の加盟店紹介に「記録町カリー食堂」を追加。牛すじとピコぬネギを使った「牛すじネギカレー」が名物のカレー専門店
---

## 2026-08-26（3）
---
### 追加
- `game/negi-game.html`:市役所食堂の小ネタミニゲーム「ネギ盛り限界チャレンジ」を追加。カタブツ職員の出現を避けながらネギを盛るクリッカー形式(現代病相談窓口記録ファイルの症例シリーズとは別枠)
### 変更
- `facilities/syokudo.html`:本文中にミニゲームへのリンクを追加
---
